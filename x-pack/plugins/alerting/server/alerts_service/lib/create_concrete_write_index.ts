...
import { DataStreamAdapter, getDataStreamAdapter } from './data_stream_adapter';
...
export const createConcreteWriteIndex = async (opts: CreateConcreteWriteIndexOpts) => {
  const { logger, indexPatterns } = opts;

  // use the alias data stream adapter as "legacy" default
  const dataStreamAdapter =
    opts.dataStreamAdapter || getDataStreamAdapter({ useDataStreamForAlerts: false });

  const label = dataStreamAdapter.isUsingDataStreams() ? 'data stream' : 'concrete write index';
  logger.info(`Creating ${label} - ${indexPatterns.name}`);
  await dataStreamAdapter.createStream(opts);
};
