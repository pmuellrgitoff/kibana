...
describe('validateParams()', () => {
  test('should validate and pass when params is valid', () => {
    expect(validateParams(actionType, { message: 'a message' }, mockedConfigUtils)).toEqual({
      message: 'a message',
    });
  });
...
describe('validateActionTypeSecrets()', () => {
  test('should validate and pass when config is valid', () => {
    validateSecrets(actionType, { webhookUrl: 'https://example.com' }, mockedConfigUtils);
  });
  ...
  test('should validate and throw error when config is invalid', () => {
    ...
    expect(() => {
      validateSecrets(actionType, { webhookUrl: 1 }, mockedConfigUtils);
    }).toThrowErrorMatchingInlineSnapshot(
      `"error validating action type secrets: [webhookUrl]: expected value of type [string] but got [number]"`
    );

    expect(() => {
      validateSecrets(actionType, { webhookUrl: 'fee-fi-fo-fum' }, mockedConfigUtils);
    }).toThrowErrorMatchingInlineSnapshot(
  ...
  test('should validate and pass when the slack webhookUrl is whitelisted', () => {
    ...
    expect(
      validateSecrets(actionType, { webhookUrl: 'https://api.slack.com/' }, mockedConfigUtils)
    ).toEqual({
